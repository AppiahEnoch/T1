...
class Ranking:
    ...
    def get_student_aggregate(student_id, class_id, semester_id, year):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT number_equivalence, isCore
        ...
        core_scores = sorted([int(record["number_equivalence"]) for record in records if record["isCore"] == 1])
        elective_scores = sorted([int(record["number_equivalence"]) for record in records if record["isCore"] == 0])

        best_3_core = core_scores[:3]
        best_3_elective = elective_scores[:3]
    ...
    def getRanking(self, exam_title, class_id=None, year=None, semester_id=None):
        ...
        total_columns = len(subjects) + 5  # Number of subjects plus 6 additional columns
        export_data = []
        for student in sorted_students:
            student_row = [student['student_name'], student['total'], student['average'], student['position']]
            for subject in subjects:
                student_row.insert(-3, student['subjects'].get(subject, 0))
        ...
        # Add the header
        header = ["NO.", "NAME OF STUDENT"] + subjects + ["TOTAL", "AVERAGE", "POSITION"]
        ws.append(header)
