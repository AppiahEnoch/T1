...
class AddHouse:
    def __init__(self, root):
        ...
        # House name entry
        ttk.Label(self.frame, text="Enter House Name:", font=label_font).grid(row=0, column=0, padx=2, pady=entry_padding_y)
        
        self.house_name_var = ttk.StringVar()
        self.house_name_entry = ttk.Entry(self.frame, textvariable=self.house_name_var, width=30)
        self.house_name_entry.grid(row=1, column=0, padx=15, pady=entry_padding_y, sticky=ttk.W)
        ...
        # Table setup
        columns = ("house_name", "delete")
        self.table = tkttk.Treeview(self.frame, columns=columns, show="headings", style="Custom.Treeview")
        self.table.heading("house_name", text="House Name")
        self.table.heading("delete", text="")

        self.table.column("house_name", width=300, anchor=ttk.CENTER)
    ...
    def submit(self):
        house_name = self.house_name_var.get().upper()
        if house_name:
            insert_update_table('house_name', 'house', ['house_name'], [house_name])
            self.update_table()
            self.house_name_var.set("")
    ...
    def update_table(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT house_name FROM house ORDER BY house_name ASC")
        for row in cursor.fetchall():
            self.table.insert("", "end", values=(row["house_name"], "Delete"))
        conn.close()
    ...
    def on_delete(self, event):
        ...
        if item and column == '#2':  # The 'delete' column is at index 1 (2nd column)
            house_name = self.table.item(item, "values")[0]
            if messagebox.askyesno("Delete", f"Are you sure you want to delete '{house_name}'?"):
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM house WHERE house_name = ?", (house_name,))
                conn.commit()
                conn.close()
                self.update_table()
